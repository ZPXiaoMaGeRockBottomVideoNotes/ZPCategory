...
/**
 ...
 ·原类和分类中方法的调用顺序：
 1、当原类只有一个分类，并且原类和分类中都有相同名称的实例方法或者类方法的时候，不管原类和分类的编译顺序，当原类的实例对象调用实例方法的时候，只会执行分类中的实例方法，然后整个调用过程结束。当原类的class对象调用类方法的时候，只会执行分类中的类方法，然后整个调用过程结束。
 2、当一个原类有多个分类的时候，并且原类和这些分类中都有相同名称的实例方法或者类方法的时候，当原类的实例对象调用实例方法的时候或者当原类的class对象调用类方法的时候，系统的调用顺序符合下面的两个原则：
 ①不管原类是先编译的还是后编译的，永远只执行分类中的实例方法或者类方法，然后整个调用过程结束；
 ②系统总是只执行后编译的分类里面的实例方法或者类方法，然后整个调用过程结束。
...
//    [ZPPerson classMethod];
    ...
    //分类和原类中都有相同名称的类方法，原类的类对象调用这个类方法
//    [ZPPerson jump];
}
