...
/**
 ...
 3、原类的实例对象调用分类中的实例方法时：当原类的instance对象调用分类中的实例方法"[person eat];"的时候，系统首先会根据原类的instance对象里面的isa指针找到原类的class对象，然后在这个class对象里面的实例方法列表中查找那个分类的实例方法"eat"，根据上面2所述，在程序运行的时候，Runtime运行时机制会把分类中的实例方法合并到原类的class对象里面，所以系统在原类的class对象里面能够查找到这个分类的实例方法，然后再进行调用；
 4、原类的类对象调用分类中的类方法时：当原类的class对象调用分类中的类方法"[ZPPerson classMethod];"的时候，系统首先会根据原类的class对象中的isa指针找到它的meta-class对象，然后在这个meta-class对象中的类方法列表中查找那个分类的类方法"classMethod"，根据上面2所述，在程序运行的时候，Runtime运行时机制会把分类中的类方法合并到原类的meta-class对象里面，所以系统在原类的meta-class对象里面能够查找到这个分类的类方法，然后再进行调用。
 以上就是分类的实现原理。
 备注：不存在分类的class对象，也不存在分类的meta-class对象。类的class对象和meta-class对象各只有唯一的一份。

 ·原类和分类中方法的调用顺序：
 1、当原类只有一个分类，并且原类和分类中都有相同名称的实例方法或者类方法的时候，不管原类和分类的编译顺序，都会先执行分类中的实例方法或者类方法；
 2、当一个原类有多个分类的时候，并且原类和这些分类中都有相同名称的实例方法或者类方法的时候，当调用这些方法的时候，系统的调用顺序符合下面的两个原则：
 ①不管原类是先编译的还是后编译的，永远先执行分类中的实例方法或者类方法，再执行原类中的实例方法或者类方法；
 ②系统总是先执行后编译的分类里面的实例方法或者类方法。
